...
class Palette(BaseModel):
    ...
    def get_current_visuals(self, phase='train'):
        ...
        if self.task in ['inpainting','uncropping','my_mission_mask','my_mission_mask_ddim']:
            dict.update({
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            ret_result.append(self.output[idx].detach().float().cpu()) # 原来有错误，本来因该是保存out，这里写成了process的最后一位，现在改回来了
        if self.task in ['inpainting','uncropping','my_mission_mask','my_mission_mask_ddim']:
            ...
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']: # 暂时用不上，这里的restoration先不改
                    if self.task in ['inpainting','uncropping','my_mission_mask']:
                        ...
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num,target=self.gt_image)
                else:
                    if self.task in ['inpainting','uncropping','my_mission_mask']:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num,target=self.gt_image)
                    elif self.task in ['my_mission_mask_ddim']:
                        self.output, self.visuals = self.netG.restoration_ddim(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num,target=self.gt_image)
                    else:
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,target=self.gt_image)  # 我的任务更接近color，所以暂时用这个，这一系列restoration注释进行的同一类修改
            ...
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if self.opt['distributed']:
                    ...
                    if self.task in ['inpainting','uncropping','my_mission_mask']:
                        ...
                        self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,target=self.gt_image)
