...
class Network(BaseNetwork):
    ...
    def set_new_noise_schedule(self, device=torch.device('cuda'), phase='train'):
        ...
        gammas_prev = np.append(1., gammas[:-1]) # 有可能和下面这个东西相同相当于'alphas_bar_prev_whole'，但是下面一行表示只在左边填1，这一行截掉了最后一个数，不知道会不会有影响！！！！
        self.gammas_prev = to_torch(np.append(1., gammas)) # ddim加装的
    ...
    def p_mean_variance(self, y_t, t, clip_denoised: bool, y_cond=None):
        noise_level = extract(self.gammas, t, x_shape=(1, 1)).to(y_t.device) # 原来的程序可能就有问题，从这一段程序来看，extract的是gammas正位，但是我训练的时候用的是-1位
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8, target=None):
        ...
        if target is not None:
            ...
            y_t = default(y_t, lambda: torch.randn_like(y_cond))  # 这一段是原程序，它根据输入的cond ch数量决定val时的噪声图通道数量修改为
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            if mask is not None:
                y_t = y_0*(1.-mask) + mask*y_t # 暂时没搞懂y0是什么，在这个yt只有在inpainting和uncroping存在时用到，（我的任务更接近collourization）
            if i % sample_inter == 0:
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None):
        ...
        gamma_t1 = extract(self.gammas, t, x_shape=(1, 1)) # 去除鲁棒性，从原来的restoration程序来看，应该从t采样gamma不是t-1，应改成t，对之前两个模型都有影响，因为ynoise采样基于这个
        ...
        if mask is not None:
            ...
            noise_hat = self.denoise_fn(torch.cat([y_cond, y_noisy], dim=1), sample_gammas) # 逆向过程in_ch=5u因为我的输入是4+1，但是本程序的前向过程写的有问题，其根据cond的channel数量设置噪声，所以需要修改
