...
class MultiModelAISystem:
    ...
    def __init__(self, device=None):
        ...
        self.model_configs = {
            'cbf': {
                ...
                'weight_base': os.path.join(current_dir, 'weights_palette', 'cbf', '150'),
                'use_ema': True,
                'color': '#e67e22',
            ...
            'cbv': {
                ...
                'weight_base': os.path.join(current_dir, 'weights_palette', 'cbv', '140'),
                'use_ema': True,
                'color': '#3498db',
            ...
            'tmax': {
                ...
                'weight_base': os.path.join(current_dir, 'weights_palette', 'tmax', '160'),
                'use_ema': True,
                'color': '#9b59b6',
    ...
    def load_all_models(self):
        ...
        for model_key, config in self.model_configs.items():
            ...
            print(f"   配置文件: {'✓' if config_exists else '✗'} {config['config_path']}")
...
class MRDPMModel:
    ...
    def _load_residual_weights(self):
        ...
        if not os.path.exists(self.residual_weight_path):
            raise FileNotFoundError(f"残差模型权重文件不存在: {self.residual_weight_path}")
        
        try:
            checkpoint = torch.load(self.residual_weight_path, map_location='cpu', weights_only=False)
            ...
            print(f"✓ 过滤后残差权重键数量: {len(denoise_state_dict)} (原始: {len(new_state_dict)})")
    ...
    def restoration(self, y_cond, mask, save_path=None):
        ...
        with torch.no_grad():
            ...
            if save_path:
                ...
                # 归一化到0-255用于可视化
                y_initial_normalized = (y_initial_np - y_initial_np.min()) / (y_initial_np.max() - y_initial_np.min())
    ...
    def inference(self, rgb_data, mask_data, save_path=None):
        ...
        try:
            with torch.no_grad():
                ...
                print(f"输入张量形状: cond_tensor={cond_tensor.shape}, mask_tensor={mask_tensor.shape}")
...
class MedicalAIModel:
    ...
    def load_model_direct(self):
        ...
        try:
            ...
            import sys
            palette_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'palette')
    ...
    def preprocess(self, rgb_data, mask_data):
        ...
        try:
            ...
            # 确保掩码是二值的 [0, 1]
            mask_tensor = (mask_tensor > 0.5).float()
            print(f"掩码二值化后范围: [{mask_tensor.min():.3f}, {mask_tensor.max():.3f}]")
            print(f"掩码中1的像素比例: {torch.sum(mask_tensor > 0.5).item() / mask_tensor.numel():.3f}")
            ...
            # 验证应用掩码后的数值范围
            print(f"应用掩码后条件图像范围: [{cond_tensor.min():.3f}, {cond_tensor.max():.3f}]")
        ...
        try:
            with torch.no_grad():
                ...
                if self.expected_in_channels == 4:
                    ...
                    print(f"⚠ 未知的输入通道数配置: {self.expected_in_channels}，使用默认1通道噪声")
    ...
    def postprocess_output(self, output, mask_tensor):
        ...
        if mask_tensor is not None:
            ...
            if np.any(background_mask):
                ...
                print(f"  - 非零像素: {bg_nonzero}/{total_bg_pixels} ({bg_nonzero / total_bg_pixels * 100:.1f}%)")
