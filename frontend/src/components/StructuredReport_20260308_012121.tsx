...
export const StructuredReport: React.FC<StructuredReportProps> = ({ patientId, fileId, analysisData }) => {
  ...
  useEffect(() => {
    ...
    const loadSavedNotes = async () => {
      try {
        const res = await fetch(`/api/get_imaging/${fileId}`)
        if (!res.ok) return
        const data = await res.json()
        if (cancelled) return
        if (data?.success) {
          const savedNotes = typeof data.data?.notes === 'string' ? data.data.notes : ''
          setNotes(savedNotes || '')
        }
      } catch {
        // Ignore notes preload failure; editing and save still work.
      }
    }
