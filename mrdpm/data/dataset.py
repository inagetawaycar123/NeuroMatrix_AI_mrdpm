...
class NumpyDataset_mask(data.Dataset):
    def __init__(
        ...
        loader=pil_loader,
        mcta_phase=1,
    ):  # 新增mcta_phase参数选择要使用的mCTA时相:动脉期为第2个通道（索引1）！！！！
        self.data_root = data_root
        flist = make_dataset(data_flist)
    ...
    def __getitem__(self, index):
        ...
        # 加载原始数据
        img = self.tfs_img(
            self.loader("{}/{}/{}".format(self.data_root, "CBF", file_name))
        )
        cond_image = self.tfs_img(
            self.loader("{}/{}/{}".format(self.data_root, "NCCT_mCTA_only", file_name))
        )
        mask = self.tfs_mask(
            self.loader("{}/{}/{}".format(self.data_root, "mask", file_name))
        )
