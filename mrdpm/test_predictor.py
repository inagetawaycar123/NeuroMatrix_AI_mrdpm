...
class BRANPredictorTester:
    ...
    def _extract_test_config(self, joint_config):
        """从联合训练配置中提取测试所需配置"""
        config = {
            'data_root': joint_config['datasets']['test']['which_dataset']['args']['data_root'],
            'test_list': joint_config['datasets']['test']['which_dataset']['args']['data_flist'],
            'batch_size': joint_config['datasets']['test']['dataloader']['args']['batch_size'],
            'num_workers': joint_config['datasets']['test']['dataloader']['args']['num_workers'],
            'mcta_phase': joint_config['datasets']['test']['which_dataset']['args'].get('mcta_phase', 1),
    ...
    def _load_model_weights(self, model_path):
        ...
        if model_path.endswith('best_model.pth'):
            ...
            self.logger.info(f"✅ 从检查点加载权重，epoch: {checkpoint.get('epoch', '未知')}")
        else:
            # 加载纯权重文件
            if isinstance(self.model, nn.DataParallel):
    ...
    def _setup_logging(self):
        ...
        log_file = os.path.join(log_dir, f'test_predictor_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ...
    def _validate_data_shape(self, test_loader):
        ...
        if cond_shape[1] != 3:
            self.logger.error(f"❌ 测试数据通道数不匹配: 期望3通道，实际{cond_shape[1]}通道")
                ...
                for i in range(batch_size):
                    try:
                        ...
                        if sample_idx >= len(original_filenames):
                            self.logger.warning(f"⚠️ 样本索引{sample_idx}超出文件列表范围")
                            continue
    ...
    def calculate_metrics(self):
        ...
        # 收集所有预测和真实文件
        pred_files = [f for f in os.listdir(self.output_dir) if f.startswith('Pre_') and f.endswith('.npy')]
        gt_files = [f for f in os.listdir(self.output_dir) if f.startswith('GT_') and f.endswith('.npy')]
