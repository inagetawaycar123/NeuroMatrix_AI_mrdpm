...
class StrokeAnalysis:
    ...
    def apply_hemisphere_processing(self, image, mask, hemisphere):
        ...
        if hemisphere == 'left':
            ...
            return analysis_region, analysis_mask, (0, height // 2), (height // 2, height)

        elif hemisphere == 'right':
            # 右脑：取下半部分进行分析，上半部分保留但不分析
            analysis_region = image[height // 2:, :]
            analysis_mask = mask[height // 2:, :] if mask is not None else None
            return analysis_region, analysis_mask, (height // 2, height), (0, height // 2)

        else:  # 'both'
            # 双侧：整个图像都分析
            return image, mask, (0, height), None

    def reconstruct_full_image(self, analysis_result, analysis_coords, other_coords, original_shape):
        """
        将分析结果重建为完整图像
        """
    ...
    def analyze_slice(self, tmax_data, mask_data, slice_id, hemisphere='both', output_dir=None):
        """
        分析单个Tmax切片
        """
        try:
            ...
            # 应用偏侧处理
            tmax_analysis, mask_analysis, analysis_coords, other_coords = self.apply_hemisphere_processing(
                tmax_data, mask_data, hemisphere
            )
            ...
            # 后处理
            penumbra_clean_pred = self.postprocess_mask(penumbra_mask_pred, self.penumbra_min_area_pred)
            core_clean_pred = self.postprocess_mask(core_mask_pred, self.core_min_area_pred)
    ...
    def generate_visualizations(self, original_image, penumbra_mask, core_mask, slice_id, output_dir):
        ...
        try:
            ...
            try:
                ...
                # Alpha通道
                combined_mask[..., 3] = np.maximum(
                    (penumbra_mask > 0).astype(float),
                    (core_mask > 0).astype(float)
                ) * 0.7
    ...
    def calculate_volumes(self, penumbra_voxels, core_voxels):
        ...
        return {
            'penumbra_volume_ml': penumbra_volume,
            'core_volume_ml': core_volume
        }
    ...
    def generate_report(self, analysis_results):
        ...
        report = {
            'summary': {
                ...
                'penumbra_volume_ml': analysis_results['volume_analysis']['penumbra_volume_ml'],
                'core_volume_ml': analysis_results['volume_analysis']['core_volume_ml'],
                'mismatch_ratio': analysis_results['mismatch_analysis']['mismatch_ratio'],
...
def analyze_stroke_case(file_id, hemisphere='both', output_base_dir=None):
    ...
    try:
        ...
        try:
            ...
            slice_files = [f for f in all_files if f.startswith('slice_') and f.endswith('_tmax_output.npy')]
        except Exception as e:
            print(f"✗ 读取目录失败: {e}")
        ...
        if os.path.exists(analysis_output_dir):
            ...
            for slice_id in range(len(tmax_slices)):
                penumbra_path = os.path.join(analysis_output_dir, f"penumbra_{slice_id}.png")
                core_path = os.path.join(analysis_output_dir, f"core_{slice_id}.png")
                combined_path = os.path.join(analysis_output_dir, f"combined_{slice_id}.png")
